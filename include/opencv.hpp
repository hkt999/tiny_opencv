...
class Mat
{
	...
	public:
		...
		static Mat& zeros(int cols, int rows, int type);
		static Mat& ones(int cols, int rows, int type);
		static Mat& eye(int rows, int cols, int type);
		template <typename T> void setIdentity(T value) {
	...
	public:
		// member functions
		inline bool empty() { return (data == 0); }
